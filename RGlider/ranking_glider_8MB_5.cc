...
//Source code for configs 3 and 4
...
#define SAMPLED_CACHE_SIZE 2800*NUM_CORE
...
uint32_t GetVictimInSet (uint32_t cpu, uint32_t set, const BLOCK *current_set, uint64_t PC, uint64_t paddr, uint32_t type)
{
    ...
    for (uint32_t i=0; i<LLC_WAYS; i++) {
        if (prefetched[set][i]) {
            curr_prediction = prefetch_predictor->get_value(PC,curr_pc_hist, cache_pc_hist[set][i]);
            ...
            curr_prediction = demand_predictor->get_value(PC,curr_pc_hist, cache_pc_hist[set][i]);
            if (curr_prediction > max_prediction) {
                max_prediction = curr_prediction;
                lru_victim = i;
            }
    ...
    if( SAMPLED_SET(set) && max_prediction < 0)
    {
        ...
        if(prefetched[set][lru_victim])
            prefetch_predictor->decrement(PC,curr_pc_hist,victim_pc_hist);
        else
            demand_predictor->decrement(PC,curr_pc_hist,victim_pc_hist);
    }
    return lru_victim;
}
...
void UpdateReplacementState (uint32_t cpu, uint32_t set, uint32_t way, uint64_t paddr, uint64_t PC, uint64_t victim_addr, uint32_t type, uint8_t hit)
{
    ...
    if(SAMPLED_SET(set))
    {
        ...
        if((addr_history[sampler_set].find(sampler_tag) != addr_history[sampler_set].end()) && (type != PREFETCH))
        {
            ...
            if( !wrap && perset_optgen[set].should_cache(curr_quanta, last_quanta))
            {
                if(addr_history[sampler_set][sampler_tag].prefetched)
                    ...
                    prefetch_predictor->increment(PC,curr_pc_hist,victim_pc_hist);
                else
                    //demand_predictor->increment(addr_history[sampler_set][sampler_tag].PC);
                    demand_predictor->increment(PC,curr_pc_hist,victim_pc_hist);
            }
            else
            {
        ...
        bool new_prediction = demand_predictor->get_prediction (PC,curr_pc_hist,victim_pc_hist);
        if (type == PREFETCH)
            new_prediction = prefetch_predictor->get_prediction (PC,curr_pc_hist,victim_pc_hist);
