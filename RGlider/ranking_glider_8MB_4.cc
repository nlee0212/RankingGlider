...
short int cache_pc_hist_lru[LLC_SETS][LLC_WAYS][k];
...
uint32_t GetVictimInSet (uint32_t cpu, uint32_t set, const BLOCK *current_set, uint64_t PC, uint64_t paddr, uint32_t type)
{
    ...
    for (uint32_t j=0; j<k; j++) {
		...
		update_pc_history_lru(cache_pc_hist_lru[set][lru_victim]);
		replace_pc_history_lru(PC,cache_pc_hist[set][lru_victim],cache_pc_hist_lru[set][lru_victim]);
    }
